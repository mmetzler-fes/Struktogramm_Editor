document.addEventListener('DOMContentLoaded', () => {
	...
	function moveBlock(blockId, targetFrom, targetTo) {
		...
		const predEdge = graphState.edges.find(e => e.to === blockId && !internalNodes.includes(e.from));
		if (!predEdge) {
		...
		console.log('[MOVE] Edges before extraction:', JSON.stringify(graphState.edges, null, 2));

		// Remove ALL EXTERNAL edges pointing TO blockId (from outside the block cluster)
		// Keep internal edges (like loop back-edges)
		...
		graphState.edges = graphState.edges.filter(e => {
			if (e.to === blockId && !internalNodes.includes(e.from)) {
				...
				return false; // Remove external edge to block
			}
			return true; // Keep everything else
		});
		console.log('[MOVE] Removed external edges to block:', removedExternal);

		// Remove ALL edges: exitNodeId → externalSuccessorId  (for loops: this carries the 'Exit' label)
		const removedExit = [];
		graphState.edges = graphState.edges.filter(e => {
		...
		graphState.edges.push(healEdge);
		console.log('[MOVE] Added heal edge:', healEdge);
		console.log('[MOVE] Edges after extraction:', JSON.stringify(graphState.edges, null, 2));
		...
		graphState.edges.push(insertEdge1);
		...
		graphState.edges.push(insertEdge2);
		console.log('[MOVE] Added exit edge:', insertEdge2);

		console.log('[MOVE] Final edges:', JSON.stringify(graphState.edges, null, 2));
